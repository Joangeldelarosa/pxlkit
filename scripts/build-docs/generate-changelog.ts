 ...
 *
 * SAFE: writes to `<package>/CHANGELOG.generated.md` — never overwrites a hand
 * authored `CHANGELOG.md`. Idempotent; deterministic output (no timestamps in
 * body beyond commit `YYYY-MM-DD`).
 *
 * CLI:
...
export function renderChangelog(pkg: ChangelogPackage): string {
  const lines: string[] = [];
...
async function discoverWorkspaces(repoRoot: string): Promise<WorkspacePackage[]> {
  ...
  for (const file of pkgFiles) {
    try {
      const pkg = (await fs.readJson(file)) as { name?: string };
      if (pkg && typeof pkg.name === "string") {
        out.push({ name: pkg.name, dir: toPosix(path.dirname(file)) });
      }
    } catch {
...
export class ChangelogGenerator extends Generator {
  ...
  override async run(ctx: GeneratorContext): Promise<GeneratorResult> {
    ...
    for (const ws of workspaces) {
      ...
      const content = renderChangelog(composed);
      const outPath = toPosix(path.join(ws.dir, "CHANGELOG.generated.md"));
